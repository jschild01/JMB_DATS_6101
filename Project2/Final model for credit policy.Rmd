...
All the coefficients are found significant (small p-values). GRE and GPA have positive effects on admission chance (credit.policy = 1), while "higher" purpose (purpose = 4 is highest) negatively affect the admission's likelihood. These are reasonable results and confirms our common beliefs.
...
-   Use the built-in type option `predict(credit.policyLogit, newdata=newdata1, type="response")` to get the *p* value: `r pred_response`
-   Use the link function value `predict(credit.policyLogit, newdata = newdata1, type = "link")` (or without the type argument, since this is the default), to calculate the value *p* `from 1/(1+exp( -linkFunctionValue ))`: `r`1/(1+exp(-pred_link)\`\
...
With the McFadden value of `r credit.policyLogitpr2['McFadden']`, which is analogous to the coefficient of determination $R^2$, only about 8% of the variations in y is explained by the explanatory variables in the model.

A major weakness of the overall model is likely from the small dataset sample size of `r length(credit.policy$credit.policy)`. We expect a much higher number of observations will increase the sensitivity of the model.
...
The p-value of `r credit.policyLogitHoslem$p.value` is relatively high. This indicates the model is not really a good fit, despite all the coefficients are significant.
