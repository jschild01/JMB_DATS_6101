...
We are using the same dataset from our first project. The dataset contains `r nrow(loans)` observations and `r ncol(loans)` variables and is based on loans from LendingClub, a peer-to-peer lending company, that were made from 2007 to 2015. Of the `r ncol(loans)` variables, 
...
We start by building the classification tree model.
...
We can see that `int.rate` was not used in the model, but `fico`, and `inq.last.6mths` were.

To add: more comments on results
...
We can plot the model to visualize it.
```{r}
...
This produces a nice visualization of the tree.
```{r}
