...
```
Of all of the loans, 753 had more greater than or equal to four `inq.last.6mths`. Of those 753:

* 622 had a `FICO` lower than 740 and failed to meet the `credit.policy`
* 11 had a `FICO` score 740 or higher and failed to meet the `credit.policy`
* 120 had a `FICO` score 740 or higher and met the `credit.policy`
...
Conversely, of all of the loans, 8825 had less than four `inq.last.6mths`. Of those 8825:

* 434 had a `FICO` lower than 660 and failed to meet the `credit.policy`
* 2 had a `FICO` lower than 660 but still managed to meet the `credit.policy`
* 801 had a `FICO` score 660 or higher and failed to meet the `credit.policy`
* 7588 had a `FICO` score 660 or higher and met the `credit.policy`
...
```
To add: can we include the image created?
...
```
Based on the Kappa statistic of `r round(confusion_matrix$overall["Kappa"], digits=2)` we would evaluate this classifier as substantial.
...
## Can we highlight the values in the first column ("Actual") using kable??
```
...
For certain statistics this model performs extremely well, with  near 100% specificity, precision, and positive prediction value. Negative prediction value is also very high at around 90%. 
