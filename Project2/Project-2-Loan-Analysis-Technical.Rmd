...
The AUC of the ROC is `r auc(tree_roc_nfp)`, which is much less than the preferred 0.8 threshold, so we would consider this to be a poor model.
...
```{r}
# Removing the added prob variable
loans <- loans %>%
  select(-prob)
```
...
# We will convert the average to an easier to read percentage by multiplying by 100, rounding, and adding a "%" at the end.
loans %>%
  mutate(vector_prediction = round(predict(credit.policyLogit, type = "response"))) %>%
  group_by(vector_prediction) %>%
  summarize(percent_not_fully_paid = paste0(round(100*mean(not.fully.paid), 1), "%"))  %>%
  ungroup() %>%
  rename(`Meets Predicted Credit Policy` = vector_prediction, `Percent Not Fully Paid` = percent_not_fully_paid) %>%
...
The cv.glmnet performs the 10 fold cross validation on our training data and picks the  best lambda value based on least mean square cross validation error criteria.
For our case, the lambda is :
```{r,results='markup'}
# Best Lambda
...
The below shows the shrunk co-efficients obtained from the model. The unimportant co-efficients goes to zero. The dots next to the co-efficient names represent the shrinkage of the unimportant variables to zero.
```{r,results='markup'}
# Co-efficients
coef(alpha1.fit)
```
