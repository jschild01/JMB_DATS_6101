...
summary(creditfit) # detailed summary of splits; show these?

```


Building the classification tree, we can see that `days.with.cr.line`, `fico`, `inq.last.6mths`, and `revol.bal` were used in the tree construction and the tree was split 5 times.
...
Our initial classification tree revealed in no chosen variables and only the single trunk node, a kappa value of 0, and a ROC curve that fit directly with the random classifier line. We interpreted this as a strong indicator that a classification tree would not be useful for `not.fully.paid`.
...
creditfit_nfp <- rpart(not.fully.paid ~ credit.policy	+ purpose + int.rate + installment + log.annual.inc + dti + fico + days.with.cr.line + revol.bal + revol.util + inq.last.6mths + delinq.2yrs + pub.rec + not.fully.paid + has.delinq.2yrs + has.pub.rec + log.revol.bal + has.revol.bal, data = loans_tree_nfp, cp=.001, method="class", control=list(maxdepth=4))

printcp(creditfit_nfp) # display the results 
plotcp(creditfit_nfp) # visualize cross-validation results
summary(creditfit_nfp) # detailed summary of splits; show these?
...
We can better visualize the tree by plotting it out. We can see that the tree is not vary useful. 


We can see the `inq.last.6mths` threshold of less than six, as well as the two thresholds of 740 and 660 or `FICO` scores. It details that of our 9578 loan sample, there are 1868 loans that failed to meet the `credit.policy` and 7710 that did. 
...
```
Our only main takeaway from this classification tree is that most loans that met the `credit.policy` were paid. We can also speculate that loans used for `home_improvement` and `small_business` with high interest rates and a high number of `inq.last.6mths` have a higher likelihood of not getting paid, which would fit generic expectations. 
...
For certain statistics this model performs relatively poorly, with 1.3% specificity, a negative predictive value of 64.5%, and a balanced accuracy of only 50.6%.  
...
The AUC of the ROC is `r auc(tree_roc)`, which is much less than the preferred 0.8 threshold, so we would consider this to be a poor model.



# Simple Linear Regression

For the 4 combinations of variables that we identified as having a notable level of correlation (greater than 0.4 or less than -0.4), it makes sense to see if we can model these relationships with linear regression.
...
```{r}
# fit linear model
linear_model <- lm(revol.util ~ log.revol.bal, data=loans)
...
#  theme_minimal()
```
From the heat-map we see that there are five significant correlations . In order to further explore that we plot 5 scatter plots with simple linear regressions. The first one is interest rate vs fico. We can see from the graph that the relation is almost linear as the linear regression line lies close to the true regression line. This is further confirmed by the adjusted r^2 value that is relatively significant at 51% and the p value at least than 0.05.

The second one is revolving utility vs interest rate. We can see from the graph that the relation is not as linear as the linear regression line lies tangent to the true regression line. This is further confirmed by the adjusted r^2 value that is relatively low at 21% and the p value at least than 0.05.

The third one is installment vs log.annual.inc. We can see from the graph that the relation is not as linear as the linear regression line lies tangent to the true regression line but converges as both variables increase. This is further confirmed by the adjusted r^2 value that is relatively low at 20% and the p value at least than 0.05.

The fourth one is revolving utility vs fico score. We can see from the graph that the relation is not as linear as the linear regression line lies relatively loser to the true regression line in comparison to the previous two cases. This is further confirmed by the adjusted r^2 value that is relatively low at 29% and the p value at least than 0.05.

In the last graph we can see that there is a strong positive relation between the two variables namely log.revol.bal and revol.util of 0.5. However,we see that since the true regression is far from linear the adjusted r^2 is relatively low at 24.85%. 
...
```{r}
mod2 <- lm(dist_new ~ fico+installment+purpose+revol.util+log.revol.bal+credit.policy+log.annual.inc, data=loans) # Fit our model to get our residuals.
loans$resi <- mod2$residuals
varfunc.ols <- lm(log(resi^2) ~ log(fico)+log(installment)+credit.policy+log.annual.inc, data = loans)
summary(varfunc.ols)
```
```{r}
varfunc.ols <- lm(log(resi^2) ~ log(fico)+log(installment)+credit.policy+log.annual.inc, data = loans)
loans$varfunc <- exp(varfunc.ols$fitted.values)
food.gls <- lm(dist_new ~ fico+installment+purpose+revol.util+log.revol.bal+credit.policy+log.annual.inc, weights = 1/sqrt(varfunc), data = loans)
summary(food.gls)
```
```{r}
lmtest::bptest(food.gls)
par(mfrow=c(2,2))
plot(food.gls)
```
...
To study the effects on admission by the factor purpose (credit.policy and purpose are both categorical variables), we can create two-way contingency table of the outcome and predictors, and make sure there are no cells of zero frequencies.\
...
A major weakness of the overall model is likely from the small dataset sample size of `r length(loans$credit.policy)`. We expect a much higher number of observations will increase the sensitivity of the model.
...
A major weakness of the overall model is likely from the small dataset sample size of `r length(loans$not.fully.paid)`. We expect a much higher number of observations will increase the sensitivity of the model.
