...
```{r}
loans_multiple_regression <- cbind(loans, dist_new=predict(distBCMod, loans$int.rate)) 
head(loans_multiple_regression) 
```
```{r}
mod1 <- lm(dist_new ~ fico+installment+purpose+revol.util+log.revol.bal+credit.policy+log.annual.inc, data=loans_multiple_regression)
summary(mod1)
lmtest::bptest(mod1)
```
