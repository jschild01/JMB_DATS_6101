...
author: "Team 1: Jonathan Schild, Medhasweta Sen, Brian Gulko, Bharat Premnath | The George Washington University"
date: "`r Sys.Date()`"
...
Since, none of the variables are quite linearly related this should mean multicolinearity shouldn't be a problem here while we build our MLR.
...
## Feature Selection
This helps us to rank the independent variables in the descending order of their importance and helps us pick the ones that we relevant in deciding the rate of interest for a loan. It is to be noted that variables like revolving balance and revolving utility isn't available at the time of deciding the rate of interest these cannot be used in the model.
...
This confirms our hypothesis that riskier the loan higher the interest rate.



# LASSO Regression for Interest Rate
...
The original categorical variables were dropped by keeping the dummy variables and used it for training and testing.
...
vip(lasso_alpha1_fit, aesthetics = list(fill = "steelblue")) +
  theme_minimal()
```

It is clear from the graph that the interest rates are heavily influenced by the compliance with the credit policy and credit score (fico).
...
This helps us to rank the independent variables in the descending order of their importance and helps us pick the ones that we relevant in deciding the credit underwriting criteria for a loan. It is to be noted that variables like revolving balance and revolving utility isn't available at the time of deciding the credit policy these cannot be used in the model.
...
This helps us rank the dependent variables in the order of their statistical significance while predicting `not.fully.paid`. Since, all the variables are available at the time of deciding if the loan can be paid back or not all the variables are applied whilebuilding the final model.
```{r}
set.seed(7)
...
The high kappa value of .859 suggests there is a high degree of agreement between the predictions and the actual values. Based on this, we would evaluate this classifier as almost perfect agreement.

The sensitivity and specificity values indicate the test can fairly reliably detect loans that meet and fail to meet the credit policy. The high precision indicates the tree is very good identifying true positives, in that 99.6% of its predicted fails were right.
...
The AUC of the ROC is `r round(auc(tree_roc),2)`, which is greater than the 0.8 threshold, so we would consider this to be a good indicator/model. We can conclude that this is overall an effective model for determining if a loan will meet the credit underwriting policy set by LendingClub.
...
Our initial classification tree for `not.fully.paid` did not choose any variables and had only a single trunk node, a kappa value of 0, and a ROC curve that fit directly on top of the random classifier line. We interpreted this as a strong indicator that a classification tree would not be useful for `not.fully.paid`.
...
This led to the classification tree using the variables `credit.policy`, `inq.last.6mths`, `int.rate`, and `purpose`. 
...
Similar to `credit.policy`, we can construct a confusion matrix and examine some of the statistics. The `Unpaid` values will function as the positive cases for the confusion matrix we put together based on the results of the classification tree model.
```{r results="show"}
# Creating the confusion matrix
...
Let's put together tables to visualize the confusion matrix.
...
We immediately see the extremely low specificity value, which indicates the tree is poor at detecting those loans that went unpaid. 
...
The AUC is `r round(auc(tree_roc_nfp),2)`, which is much less than the preferred 0.8 threshold, so we would consider this to be a poor model. This, with some of those other poor statistics, we can confidently conclude that this is overall a poor model for determining if a loan will default.
...
loans_tree %>%
  ...
  mutate(vector_prediction = case_when(
    vector_prediction == 1 ~ FALSE,
    vector_prediction == 2 ~ TRUE
  )) %>%
...
# We will convert the average to an easier to read percentage by multiplying by 100, rounding, and adding a "%" at the end.
loans %>%
  mutate(vector_prediction = as.logical(round(predict(logit_credit_model, type = "response")))) %>%
  group_by(vector_prediction) %>%
  summarize(percent_not_fully_paid = paste0(round(100*mean(not.fully.paid), 1), "%"))  %>%
  ungroup() %>%
  rename(`Meets Predicted Credit Policy` = vector_prediction, `Percent Not Fully Paid` = percent_not_fully_paid) %>%
...
We know from our logistic regression and classification tree models that the `fico` and `inq.last.6mths` variables strongly influence the likelihood of a borrorew meeting the credit underwriting criteria (`credit.policy` of `TRUE`).
...
In addition, our analysis led us to further explore the factors related to the interest rate of a loan. We hypothesized that the riskier the loan the higher is the interest rate and subsequent installment payments. This was proven true by out MLR and LASSO regression models as `credit.policy` being `TRUE` is one of the most statistically significant factors when predicting `int.rate`.


## Research Benefit

As a result of our work we can conclude that meeting the credit underwriting criteria is very important in reducing the likelihood of not fully repaying the loan. Furthermore, we now understand that `fico`, `log.annual.inc`, `purpose`, and `inq.last.6mths` are key factors in determining if borrowers met LendingClub's credit underwriting criteria, and therefore were at much lower risk of defaulting.

From this we now have a much better understanding of the risk factors that were present in P2P loans.
