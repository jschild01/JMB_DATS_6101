...
The AUC is `r auc(tree_roc_nfp)`, which is much less than the preferred 0.8 threshold, so we would consider this to be a poor model. This, with some of those other poor statistics, we can confidently conclude that this is overall a poor model for determining if a loan will default.
