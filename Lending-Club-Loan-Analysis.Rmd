...
Please note we also require packages corrplot and scale to efficiently conduct our EDA.If we require any other packages we will import them along the way. The commented print line checks is the dataset has been successfully read.
...
By examining the structure of our data, we can see that there is only one character variable which might be a factor, and some of the numeric variables look like logicals. We will transform them later for an effective analysis.We also note that there are 9,578 row or observations and 14 columns or variables. Another sub-step of understanding the structure of our data is to check if there are any null values and if all the observations are unique.

```{r results = "show"}
sapply(loans, function(x) sum(is.na(x)))
```
From the above step we can say that there are no null observations.

```{r results = "show"}
unique(loans)
```
Here, we can see that all of the 9,578 observations are unique.Now, we can move on to the next step of our EDA. 
